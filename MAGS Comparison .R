...
setwd("C:/Users/serghioi/Documents/QIB/PhD - Development and Evolution Of The Infant Skin Microbiome/Data Collection/PEARL Study Samples/Data Plots/Promega Maxwell DNA Extraction PEARL Baby Swabs Run 3/Illumina Taxa for MAGS at Diff Depths")

#Import the data.
data <-  read.csv("MAGS2.csv")

# Convert the data from wide to long format using "melt".
data_long <- melt(data, id.vars = c("Sample", "Depth"), variable.name = "MAGS")

# Set the metadata columns as factors.
data_long$Sample <- as.factor(data_long$Sample)
data_long$Depth <- as.factor(data_long$Depth)

##################################################################################################################

# First step making basic box plots.
ggplot(data_long, aes(x = Depth, y = value, shape = Depth, colour = Depth, fill = Depth)) +
  geom_boxplot(position = position_dodge(.8), outlier.shape = NA, aes(shape = Depth, colour = Depth, fill = Depth), lwd = .5)
...
ggplot(data_long, aes(x = Depth, y = value, shape = Depth, colour = Depth, fill = Depth)) +
  geom_boxplot(position = position_dodge(.8), outlier.shape = NA, aes(shape = Depth, colour = Depth, fill = Depth), lwd = .5) +
  geom_point(position = position_dodge(.8), aes(shape = Depth, colour = Depth, fill = Depth), size = 4, stroke = .5)
...
ggplot(data_long, aes(x = Depth, y = value, shape = Depth, colour = Depth, fill = Depth)) +
  ...
  theme(plot.title = element_text(size = 20),
    ...
    #Define x axis title.
    axis.text.x = element_text(size = 16, colour = "black", angle = 0, hjust = .5, vjust = .5),
